...
- 커밋/스테이징은 사용자가 요청한 경우에만 수행한다.
- 커밋 메시지는 사용자가 알아보기 쉽도록 **한글**로 작성한다.
- Markdown 문서는 **200줄 이하**를 유지한다. 초과 시 상세 내용을 새 MD 또는 관련 MD로 분리하고, 원문에는 링크만 남긴다.
- 파일 검색은 `rg`/`rg --files`를 우선 사용한다.
- 작업 규모에 따라 단독 진행 또는 팀 에이전트 진행을 선택한다. 상세는 `AGENT_TEAM.md` 참고.
...
### 코드 스타일
- 코드는 **쉽고 짧게, 알아보기 쉽게** 작성한다. 작은 분기/배열 매핑/단순 헬퍼로 충분한 기능을 과한 추상화나 긴 파싱 코드로 늘리지 않는다.
- 같은 의미의 `switch`/`if`가 반복되면 작은 배열, 명확한 메서드, 데이터 매핑으로 줄인다. 단, 읽기 어려운 축약이나 과도한 일반화는 피한다.
...
```
- 스타일 교정 시 기존 실행 로직/메서드 호출/조건문 결과를 변경하지 않는다.
...
- 씬 이동/데이터 로드 실패 시 명확한 에러 로그를 남긴다.
